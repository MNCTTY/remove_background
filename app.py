...
#                   cv2.imwrite(f'{os.path.splitext(f)[0]}_gray.png', gray)
                ...
                res = cv2.drawContours(mask_of_mask, contours, 0, (255,255,255),5)
...
#                   os.remove(f'{os.path.splitext(f)[0]}_gray.png')
        return render_template('index.html', files=[filename] + ['u2net_FINAL'+f'{os.path.splitext(filename)[0]}_final.png'])
    if filename.endswith('.mp4') or filename.endswith('.avi') or filename.endswith('.wmv') or filename.endswith('.mkv'):
        print(filename)
        print(type(filename))
        result = rmbg_video.process_video(filename)
        return render_template('index.html', video_files=[filename] + [f'{os.path.splitext(filename)[0]}_final.mp4'])
